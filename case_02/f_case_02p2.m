function op = f_case_02p2(ip)

big_output = 0;


Kh = ip.Kh;            % horizontal dispersion coefficient in m2/s
Kv = ip.Kv;            % vertical dispersion coefficient in m2/s
DIC_cf = ip.DIC_cf;    % DIC counterfactual [mmol /m3]
DIC_deep = ip.DIC_deep; % deep DIC (same for real and counterfactual) [mmol /m3]
...
d = Kh*dt/dx/dx;    % non-dimensional dispersion number 
...
DIC_pi_cf = pi*DIC_cf*ones(nt,nx); % counterfactual DIC
DIC_pi_deep = DIC_deep*pi;
...
for i=2:nt
    ...
    DIC_cf_diff_us = d*(-DIC_pi_cf(i-1,2:nx-1)+DIC_pi_cf(i-1,1:nx-2));
    DIC_cf_diff_ds = d*(DIC_pi_cf(i-1,3:nx)-DIC_pi_cf(i-1,2:nx-1));
    ...
    DIC_pi_cf(i,2:nx-1) = DIC_pi_cf(i-1,2:nx-1) + DIC_cf_diff_us + DIC_cf_diff_ds;
...
op(res).DIC_cf_end = DIC_pi_cf(end,:)/pi; % only save final distribution
...
for res=2:3
    ...
    DIC_cf_hr = op(res-1).DIC_cf_end;
    alk_hr    = op(res-1).alk_end;
    for i=1:length(x_hr)
        ind = find(x == x_hr(i));
        DIC_pi(1,ind)    = DIC_hr(i)*pi;
        DIC_pi_cf(1,ind) = DIC_cf_hr(i)*pi;
        alk_pi(1,ind)    = alk_hr(i)*pi;
    end

    disp('Take time of 2nd/3rd time stepping loop.')
    tic
    % the time stepping loop
    for i=2:nt
        ...
        %
        pCO2_cf = pCO2_fun(DIC_pi_cf(i-1,:)/pi,alk_cf*ones(1,nx),ip.pCO2_dict);
        dpCO2 = (ip.pCO2_air - pCO2_cf)*1.e-6; % delta pCO2  [atm]        
        Fair_sea_cf(i,:) = ip.kgas*ip.Ko*dpCO2; % [mmol / m2 /s] 
        DIC_pi_cf(i,:) = DIC_pi_cf(i-1,:) + dt*pi*Fair_sea_cf(i,:)/dz;
        ...
        DIC_pi_cf(i,2:nx-1) = DIC_pi_cf(i,2:nx-1) + DIC_cf_diff_us + DIC_cf_diff_ds;
        alk_pi(i,2:nx-1) = alk_pi(i-1,2:nx-1) + alk_diff_us + alk_diff_ds;
    %   this is equal to:    
    %   C(2:nx-1) = Cold(2:nx-1) + d*(-Cold(2:nx-1)+Cold(1:nx-2)+Cold(3:nx)-Cold(2:nx-1));

        % vertical mixing
        DIC_pi(i,:)    = DIC_pi(i,:)    - vmix*(DIC_pi(i-1,:) - DIC_pi_deep*ones(1,nx));
        DIC_pi_cf(i,:) = DIC_pi_cf(i,:) - vmix*(DIC_pi_cf(i-1,:) - DIC_pi_deep*ones(1,nx));
        alk_pi(i,:)    = alk_pi(i,:)    - vmix*(alk_pi(i-1,:) - alk_pi_cf*ones(1,nx));
         
    end		% end of time stepping loop
    toc

    % calculate CDR(t) in mol
    CDR = sum(DIC_pi/pi-DIC_pi_cf/pi,2)*dx*dz;
    F_int = sum(Fair_sea,2)*dx;
    F_cf_int = sum(Fair_sea_cf,2)*dx;
    
    % index 2 indicates 2nd resolution level (coarser)
    if big_output
        op(res).DIC = DIC_pi/pi; 
        op(res).DIC_cf = DIC_pi_cf/pi; 
    ...
    op(res).F_int = F_int;   % 
    op(res).F_cf_int = F_cf_int;   % 
    op(res).t = t;
    op(res).x = x;
end

% saving key input variables into the first field of the output structure
op(1).DIC_cf   = ip.DIC_cf ;
op(1).DIC_deep   = ip.DIC_deep ;
op(1).alk_cf   = ip.alk_cf ;
op(1).dalk   = ip.dalk ;
op(1).T   = ip.T ;
op(1).S   = ip.S ;
op(1).pCO2_air   = ip.pCO2_air ;
op(1).U10   = ip.U10 ;
op(1).dz   = ip.dz ;
op(1).Kh   = ip.Kh ;
op(1).Kv   = ip.Kv ;
